...
def m_cubo(x, qz_x, px_zs, zss):
    lpz = (
        dist.Normal(
            torch.zeros(1, zss.size(2)).cuda(), torch.ones(1, zss.size(2)).cuda()
        )
        .log_prob(zss)
        .sum(-1)
    )
    ...
    if "brain" in px_zs.keys() and "image" in px_zs.keys() and "text" in px_zs.keys():
        ...
        "brain" not in px_zs.keys()
        and "image" in px_zs.keys()
        and "text" in px_zs.keys()
    ):
        lpx_z1 = px_zs["image"].log_prob(x["image"]).sum(-1)
        lpx_z2 = px_zs["text"].log_prob(x["text"]).sum(-1)
...
def log_li(x_var, dist_info):
    ...
    logli = (
        -0.5 * torch.log(2 * pi) - torch.log(std + TINY) - 0.5 * torch.pow(epsilon, 2)
    )
    return logli.sum(1)
...
def mutual_info(exp, px_zs, z):
    if "brain" in px_zs.keys() and "image" in px_zs.keys() and "text" in px_zs.keys():
        ...
        "brain" in px_zs.keys()
        and "image" not in px_zs.keys()
        and "text" not in px_zs.keys()
    ):
        q1 = exp.Q1(px_zs["brain"].loc)
        mi1 = log_li(z, q1).mean()
        return mi1
...
def calc_log_probs(exp, result, batch):
    ...
    for m, m_key in enumerate(mods.keys()):
        if m_key in batch[0].keys():
            mod = mods[m_key]
            log_probs[mod.name] = -mod.calc_log_prob(
                result["rec"][mod.name], batch[0][mod.name], exp.flags.batch_size
            )
...
def negative_sample_generator(batch_new, batch, case):
    ...
    data = batch
    if (
        "brain" in batch_d.keys()
        and "image" in batch_d.keys()
        and "text" in batch_d.keys()
    ):
        if case == 1:
        ...
        for m, m_key in enumerate(mods.keys()):
            ...
            if exp.flags.poe_unimodal_elbos:
                if m_key == "brain" and "brain" in batch_d.keys():
                    ...
                    log_prob_mod = -mod.calc_log_prob(
                        r_mod["rec"][m_key], batch_d[m_key], exp.flags.batch_size
                    )
...
def elbo_contrast(exp, batch, epoch):
    ...
    out_basic_routine = dict()
    out_basic_routine["elbo_nega_sample_loss"] = (
        torch.log(elbo_scale.exp().sum() * exp.flags.batch_size / neg_batch_size + TINY)
        * CONSTANT
    )
    return out_basic_routine
...
def train_aug(epoch, exp, tb_logger):
    ...
    if "image_text" in exp.flags.aug_type:
        ...
        for iteration, batch in enumerate(aug_loader):
            ...
            elbo_nega_sample_loss_case_1 = basic_routine_contrast[
                "elbo_nega_sample_loss"
            ]

            neg_batch_new = negative_sample_generator(batch_new, batch, case=2)
            basic_routine_contrast = elbo_contrast(exp, neg_batch_new, epoch)
            elbo_nega_sample_loss_case_2 = basic_routine_contrast[
                "elbo_nega_sample_loss"
            ]

            elbo_nega_sample_loss = (
                elbo_nega_sample_loss_case_1 + elbo_nega_sample_loss_case_2
            )
...
def train(epoch, exp, tb_logger):
    ...
    for iteration, batch in enumerate(d_loader):
        ...
        elbo_nega_sample_loss = (
            elbo_nega_sample_loss_case_1
            + elbo_nega_sample_loss_case_2
            + elbo_nega_sample_loss_case_3
            + elbo_nega_sample_loss_case_4
            + elbo_nega_sample_loss_case_5
            + elbo_nega_sample_loss_case_6
        )
        ...
        tb_logger.write_training_logs(
            results, total_loss, log_probs, klds, -inter_mi_loss
        )
        print(
            "====> Epoch: {:03d} Train loss: {:.4f} ELBO: {:.4f} IntraMI: {:.4f} InterMI: {:.4f}".format(
                epoch, total_loss, -elbo_loss, -intra_mi, -inter_mi_loss
            )
        )
...
def test(epoch, exp, tb_logger):
    with torch.no_grad():
        ...
        for iteration, batch in enumerate(d_loader):
            ...
            neg_batch_new = negative_sample_generator(batch_new, batch, case=3)
            basic_routine_contrast = elbo_contrast(exp, neg_batch_new, epoch)
            elbo_nega_sample_loss_case_3 = basic_routine_contrast[
                "elbo_nega_sample_loss"
            ]

            neg_batch_new = negative_sample_generator(batch_new, batch, case=4)
            basic_routine_contrast = elbo_contrast(exp, neg_batch_new, epoch)
            elbo_nega_sample_loss_case_4 = basic_routine_contrast[
                "elbo_nega_sample_loss"
            ]

            neg_batch_new = negative_sample_generator(batch_new, batch, case=5)
            basic_routine_contrast = elbo_contrast(exp, neg_batch_new, epoch)
            elbo_nega_sample_loss_case_5 = basic_routine_contrast[
                "elbo_nega_sample_loss"
            ]

            neg_batch_new = negative_sample_generator(batch_new, batch, case=6)
            basic_routine_contrast = elbo_contrast(exp, neg_batch_new, epoch)
            elbo_nega_sample_loss_case_6 = basic_routine_contrast[
                "elbo_nega_sample_loss"
            ]
            ...
            inter_mi_loss = elbo_loss + elbo_nega_sample_loss / 6.0
            total_loss = total_loss + lambda2 * inter_mi_loss
            tb_logger.write_testing_logs(
                results, total_loss, log_probs, klds, -inter_mi_loss
            )
            print(
                "====> Epoch: {:03d} Test loss: {:.4f} ELBO: {:.4f} IntraMI: {:.4f} InterMI: {:.4f}".format(
                    epoch, total_loss, -elbo_loss, -intra_mi, -inter_mi_loss
                )
            )
...
def image_text_inference(exp, type):
    with torch.no_grad():
        ...
        if type == "zsl":
            image_text_data = {
                "image": exp.dataset_test.tensors[1].cuda(),
                "text": exp.dataset_test.tensors[2].cuda(),
            }
            label = exp.dataset_test.tensors[3]
            brain = exp.dataset_test.tensors[0]
        elif type == "normal":
            image_text_data = {
                "image": exp.dataset_val.tensors[1].cuda(),
                "text": exp.dataset_val.tensors[2].cuda(),
            }
...
def brain_inference(exp, type):
    with torch.no_grad():
        ...
        return (
            z.cpu().numpy(),
            label.cpu().numpy(),
            image_rec.loc.cpu().numpy(),
            text_rec.loc.cpu().numpy(),
            image.cpu().numpy(),
            text.cpu().numpy(),
        )

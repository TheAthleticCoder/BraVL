...
from brain_image_text.networks.MLP_Brain import (
    EncoderBrain,
    DecoderBrain,
)
...
"""
 Defines a class called BrainImageText which appears to set up an experiment for multimodal learning 
 involving brain imaging data, image features, and text features. 
 Importantly:
    1. `set_rec_weights`: sets the weights for the reconstruction loss of each modality (brain, image, text). 
    Reconstruction loss measures how well the model can reconstruct the input data from the latent representation.
    2. `set_style_weights`: sets the weights for the style loss of each modality. Style loss typically captures aspects of the data 
    that are related to its stylistic properties or variations (e.g., texture, color, writing style) rather than its content.
"""
...
class BrainImageText(BaseExperiment):
    ...
    def set_modalities(self):
        mod1 = Modality(
            "brain",
            EncoderBrain(self.flags),
            DecoderBrain(self.flags),
            self.flags.class_dim,
            self.flags.style_m1_dim,
            "normal",
        )
        mod2 = Modality(
            "image",
            EncoderImage(self.flags),
            DecoderImage(self.flags),
            self.flags.class_dim,
            self.flags.style_m2_dim,
            "normal",
        )
        mod3 = Modality(
            "text",
    ...
    def set_dataset(self):
        ...
        image_dir_train = os.path.join(
            data_dir_root, "visual_feature/ImageNetTraining", image_model + "-PCA", sbj
        )
        image_dir_test = os.path.join(
            data_dir_root, "visual_feature/ImageNetTest", image_model + "-PCA", sbj
        )
        text_dir_train = os.path.join(
            data_dir_root, "textual_feature/ImageNetTraining/text", text_model, sbj
        )
        text_dir_test = os.path.join(
            data_dir_root, "textual_feature/ImageNetTest/text", text_model, sbj
        )

        train_brain = sio.loadmat(
            os.path.join(brain_dir, "fmri_train_data" + stability_ratio + ".mat")
        )["data"].astype("double")
        train_image = sio.loadmat(os.path.join(image_dir_train, "feat_pca_train.mat"))[
            "data"
        ].astype("double")
        train_text = sio.loadmat(os.path.join(text_dir_train, "text_feat_train.mat"))[
            "data"
        ].astype("double")
        train_label = sio.loadmat(
            os.path.join(brain_dir, "fmri_train_data" + stability_ratio + ".mat")
        )["class_idx"].T.astype("int")
        ...
        test_brain = sio.loadmat(
            os.path.join(brain_dir, "fmri_test_data" + stability_ratio + ".mat")
        )["data"].astype("double")
        test_image = sio.loadmat(os.path.join(image_dir_test, "feat_pca_test.mat"))[
            "data"
        ].astype("double")
        test_text = sio.loadmat(os.path.join(text_dir_test, "text_feat_test.mat"))[
            "data"
        ].astype("double")
        test_label = sio.loadmat(
            os.path.join(brain_dir, "fmri_test_data" + stability_ratio + ".mat")
        )["class_idx"].T.astype("int")

        if self.flags.aug_type == "image_text":
            image_dir_aug = os.path.join(
                data_dir_root, "visual_feature/Aug_1000", image_model + "-PCA", sbj
            )
            text_dir_aug = os.path.join(
                data_dir_root, "textual_feature/Aug_1000/text", text_model, sbj
            )
            aug_image = sio.loadmat(os.path.join(image_dir_aug, "feat_pca_aug.mat"))[
                "data"
            ].astype("double")
            aug_text = sio.loadmat(os.path.join(text_dir_aug, "text_feat_aug.mat"))[
                "data"
            ].astype("double")
        ...
        if self.flags.test_type == "normal":
            ...
            train_brain, val_brain, train_label, val_label = train_test_split(
                train_brain,
                train_label_stratify,
                test_size=0.2,
                stratify=train_label_stratify,
            )
        ...
        train_dataset = torch.utils.data.TensorDataset(
            train_brain, train_image, train_text, train_label
        )
        test_dataset = torch.utils.data.TensorDataset(
            test_brain, test_image, test_text, test_label
        )
        ...
        if self.flags.test_type == "normal":
            val_dataset = torch.utils.data.TensorDataset(
                val_brain, val_image, val_text, val_label
            )
    ...
    def set_optimizer(self):
        ...
        optimizer_Qnet = optim.Adam(
            itertools.chain(
                self.Q1.parameters(), self.Q2.parameters(), self.Q3.parameters()
            ),
            lr=self.flags.initial_learning_rate,
            betas=(self.flags.beta_1, self.flags.beta_2),
        )
        ...
        scheduler_mvae = optim.lr_scheduler.StepLR(
            optimizer_mvae, step_size=20, gamma=1.0
        )
        scheduler_Qnet = optim.lr_scheduler.StepLR(
            optimizer_Qnet, step_size=20, gamma=1.0
        )
